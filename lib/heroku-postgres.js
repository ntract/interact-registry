...
if (process.env.DATABASE_URL) {
	...
	Package.sync().then( function () {
		var addIndex = sequelize.getQueryInterface().addIndex('Package', ['name']);
	...
	events.on("bower:hits", function(packageName) {
		Package.find({where: ["name = ?", packageName]}).then(function(pkg) {
			if (!pkg) return;
