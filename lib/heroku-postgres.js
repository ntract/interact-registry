console.log("Searching for heroku postgres red...");
if (process.env.HEROKU_POSTGRESQL_RED_URL) {
	...
	event.on("bower:loaded", function Loaded() {
		...
		Package.findAll({
			...
			for (var k in index) {
				if (!index2[k]) {
					event.emit("bower:register", index[k]);
				}
				index2[k] = index[k];
			}
	...
	event.on("bower:register", function Register(entry) {
	...
	event.on("bower:unregister", function Register(packageName) {
